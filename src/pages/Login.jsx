  ...
  return (
    <div className='w-screen h-screen flex justify-center items-center'>
        <main className='p-4 rounded-lg shadow-md flex flex-col space-y-2 max-sm:w-full max-sm:mx-8'>
            ...
            <button className='bg-primary text-white disabled:brightness-50 disabled:hover:cursor-not-allowed' disabled={isDisabled()} onClick={clickHandler}>Login</button>
            <button className='hover:underline' onClick={()=>navigate("/signup")}>Create A New Account</button>
        </main>
    </div>
  )
}
